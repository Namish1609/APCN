...
from PyQt6.QtWidgets import (
    QApplication, QHBoxLayout, QHeaderView, QLabel, QMessageBox,
...
class ConsolidationComparison(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.setFont(QFont("Sans Serif", 8)); x = 54
...
class ConsolidationWorker(QThread):
    ...
    def __init__(self, session: CognitiveSessionV11, *, visual_test: int, language_test: int,
        ...
        self.visual_test = int(visual_test); self.language_test = int(language_test)
        self.visual_train = int(visual_train); self.language_train = int(language_train)
    ...
    def run(self):
        try:
            ...
            self.progress.emit(58, "targeted language construction/reference learning")
            ...
            result = {
                "before": before, "after": after, "visual_training": vt,
...
class APCNV11Window(APCNV102Window):
    def __init__(self, seed: int = 11):
        ...
        if (out / "session_v0_11.json").exists():
            self.cognitive = CognitiveSessionV11.load_checkpoint(out, seed=seed)
        else:
        ...
        self._last_consolidation_refresh = 0.0

        self.tabs.insertTab(3, self._consolidation_tab(), "Consolidation")
        self.tabs.currentChanged.connect(self._on_tab_changed)
        self._refresh_competence(); self._refresh_definition_table(); self._refresh_header()
    ...
    def _consolidation_tab(self) -> QWidget:
        ...
        tl.addWidget(self.c_run, 1); tl.addWidget(self.c_migrate); outer.addWidget(top)

        self.c_progress = QProgressBar(); self.c_progress.setRange(0, 100); self.c_progress.setValue(0)
        self.c_progress.setFormat("waiting"); outer.addWidget(self.c_progress)

        main = QSplitter(Qt.Orientation.Horizontal); left = QSplitter(Qt.Orientation.Vertical)
        ...
        self.c_priorities.verticalHeader().setVisible(False); pl.addWidget(self.c_priorities); left.addWidget(priority_card)

        compare_card = self._card(); cl = QVBoxLayout(compare_card); cl.setContentsMargins(7,7,7,7)
        self.c_curve = ConsolidationComparison(); cl.addWidget(self.c_curve); left.addWidget(compare_card); left.setSizes([310, 260])
        main.addWidget(left)
        ...
        self.c_graph = SpikingGraph("UNIFIED CONCEPT GRAPH"); gl.addWidget(self.c_graph); right.addWidget(graph_card)
        audit_card = self._card(); al = QVBoxLayout(audit_card); al.setContentsMargins(7,7,7,7)
        al.addWidget(QLabel("Memory + discourse audit")); self.c_audit = QPlainTextEdit(); self.c_audit.setReadOnly(True)
        al.addWidget(self.c_audit); right.addWidget(audit_card); right.setSizes([330, 240])
        main.addWidget(right); main.setSizes([790, 530]); outer.addWidget(main, 1)
        return page
    ...
    def _graph_trace(self) -> Dict[str, object]:
        ...
        selected = {n.id for n in rows}; nodes = []
        for n in rows:
        ...
        edges = [{"src": e.src, "dst": e.dst, "weight": e.weight, "kind": e.relation}
                 for e in graph.edges.values() if e.src in selected and e.dst in selected]
        return {"nodes": nodes, "edges": edges}

    @staticmethod
    def _format_audit(a: Dict[str, object]) -> str:
        proto = a["visual_prototypes"]; err = a["error_memory"]; graph = a["concept_graph"]
        disc = a["discourse_working_memory"]; cons = a["language_constructions"]
        return (
    ...
    def _refresh_consolidation(self):
        if not hasattr(self, "c_priorities"): return
        ...
        audit = self.cognitive.memory_audit()
        self.c_audit.setPlainText(self._format_audit(audit)); self.c_graph.set_trace(self._graph_trace())
        self.c_curve.set_history(self.cognitive.consolidation_history)
        self._last_consolidation_refresh = time.monotonic()

    def _maybe_refresh_consolidation(self):
        if not hasattr(self, "c_audit") or self.tabs.currentIndex() != 3: return
        if time.monotonic() - self._last_consolidation_refresh >= 1.8:
            self._refresh_consolidation()

    def _on_tab_changed(self, index: int):
        if index == 3: self._refresh_consolidation()
    ...
    def _start_consolidation(self):
        ...
        worker = ConsolidationWorker(self.cognitive,
    ...
    def _consolidation_failed(self, text: str):
        self.c_run.setEnabled(True); self.c_migrate.setEnabled(True); self.c_progress.setFormat("failed")
        QMessageBox.critical(self, "Consolidation failed", text)

    def _migrate_v010(self):
        if self.p_timer.isActive() or self.l_timer.isActive():
            QMessageBox.information(self, "Pause training first", "Pause training before replacing the active memory."); return
        ...
        if not found:
            QMessageBox.warning(self, "No V0.10 checkpoint found",
                "Expected outputs/v0_10/perception/concept_memory_v0_8.json and/or outputs/v0_10/language_memory_v0_10.json. Finish/save a V0.10 batch first, or migrate from the CLI with explicit paths."); return
        self.cognitive = CognitiveSessionV11.from_memories(seed=self.seed,
        ...
        QMessageBox.information(self, "Migration complete", f"Imported compact memory from:\n" + "\n".join(found) +
    ...
    def _run_visual_test_v101(self):
        try: rep = self.cognitive.test_visual(self.v_count.value(), float(self.v_diff.currentData()))
        except Exception as exc: QMessageBox.critical(self, "Visual test failed", str(exc)); return
        ...
        self.visual_history = self.cognitive.visual_test_history; self.v_graph.set_history(self.visual_history); self._refresh_consolidation()
    ...
    def _run_language_test_v101(self):
        ...
        self.lt_graph.set_history(self.cognitive.language_test_history); self._refresh_consolidation()

    def _language_tick(self):
        super()._language_tick(); self._maybe_refresh_consolidation()

    def _perception_tick(self):
        super()._perception_tick(); self._maybe_refresh_consolidation()

    def _refresh_header(self):
        if not hasattr(self, "memory_label"): return
        errors = getattr(getattr(self, "cognitive", None), "errors", None); ecount = len(errors.signatures) if errors is not None else 0
        self.memory_label.setText(f"visual {self.perception.learner.episode_count} • language {self.cognitive.language.learner.episode_count} • definitions {self.cognitive.concepts.definition_count} • error signatures {ecount}")


def run_app(seed: int = 11) -> int:
    import sys
    app = QApplication.instance() or QApplication(sys.argv); app.setStyleSheet(STYLE)
    win = APCNV11Window(seed); win.show(); return app.exec()
