...
class SelfFaceMemory:
    ...
    def __init__(self,max_views: int=12):
        self.encoder=FineAppearanceEncoder(normalized_size=48,luminance_size=18,chromatic_size=12,gradient_size=14,occupancy_size=10)
    ...
    @classmethod
    def _square_crop(cls,frame: np.ndarray,bbox:BBox,pad: float=.16)->np.ndarray:
    ...
    def descriptor(self,frame: np.ndarray,bbox:BBox)->np.ndarray:
        crop=self._square_crop(frame,bbox); mask=np.ones(crop.shape[:2],dtype=np.uint8)*255
