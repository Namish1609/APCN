...
from apcn_v15.dialogue_learning_v151 import DialogueActLearnerV151
...
class TestV015(unittest.TestCase):
    ...
    def test_dialogue_development_split_is_read_only(self):
        s = self._session()
        before = s.dialogue_learner_v15.observations
        rep = s.test_dialogue_generalization(240)
        self.assertTrue(rep["memory_frozen"])
        self.assertEqual(rep["benchmark_role"], "development_not_blind")
        self.assertEqual(s.dialogue_learner_v15.observations, before)

    def test_balanced_bootstrap_uses_no_heldout_examples(self):
        s = self._session()
        self.assertEqual(s.dialogue_bootstrap_v15["held_out_examples_used"], 0)
        self.assertGreater(s.dialogue_bootstrap_v15["observations_added"], 0)

    def test_dialogue_statistics_do_not_create_world_knowledge(self):
        s = CognitiveSessionV15(seed=15005)
        self.assertFalse(s.concepts.understanding("glorpnax").get("known"))
        s.language_only_train(200)
        self.assertFalse(s.concepts.understanding("glorpnax").get("known"))

    def test_concept_knowledge_survives_router_reset(self):
        s = self._session()
        self.assertTrue(s.concepts.understanding("acceleration").get("known"))
        s.dialogue_learner_v15 = DialogueActLearnerV151()
        s.conversation = s._make_conversation()
        self.assertTrue(s.concepts.understanding("acceleration").get("known"))
    ...
    def test_conversational_memories_persist_without_raw_chat(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            audit = restored.conversation_memory_audit()
            self.assertFalse(audit["raw_chat_transcript_persisted"])
            self.assertFalse(audit["raw_corpus_memory_enabled"])

    def test_conversation_benchmark_and_architecture_contract(self):
        ...
        self.assertEqual(rep.knowledge_language_separation, 1.0)
    ...
    def test_v15_ui_has_no_corpus_exposure_surface(self):
        ...
        self.assertNotIn("install_english_exposure_panel", launcher)
        self.assertFalse(Path("apcn_v15/corpus.py").exists())
        self.assertFalse(Path("apcn_v15/corpus_ui.py").exists())
