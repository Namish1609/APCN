...
class SelfOrganizingPatchSensor:
    """Generic visual front-end based on focused pixels and an online codebook.

    No color or shape label is visible here. In particular, V0.12 removes the
    old geometric Hu/fill/aspect summary from the primary classifier. It retains
    generic photometric statistics because discarding continuous channel
    relationships proved to be an unnecessary information bottleneck.

    Explicit inductive biases remain: joint attention, translation/scale
    normalization, coarse pose normalization, generic photometric statistics,
    local patches and spatial pooling. This is less handcrafted perception, not
    a claim of learning vision from literally zero prior structure.
    """

    VERSION = "APCN-V0.12-SELF-ORGANIZING-PATCH-SENSOR"

    def __init__(self, normalized_size: int = 32, raster_size: int = 10,
                 histogram_bins: int = 8, chromatic_bins: int = 10,
        ...
        # Photometric block:
        #   3 raw channel histograms
        #   raw mean/std (6 continuous values)
        #   normalized chromatic mean/std (6 continuous values)
        #   joint 2-D normalized-channel histogram
        # None of these dimensions has a semantic color name.
        photometric = (
            3 * self.histogram_bins
            + 12
            + self.chromatic_bins * self.chromatic_bins
        )
        raster_end = photometric + self.raster_size * self.raster_size
        code_end = raster_end + 5 * self.max_codewords
        self.layout = FeatureLayout(code_end, {
            "pixel_distribution": (0, photometric),
            "normalized_raster": (photometric, raster_end),
            "learned_patch_codebook": (raster_end, code_end),
        })
    ...
    def _prepare(self, image: np.ndarray, attention_mask: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Generic pose normalization. Principal direction is intentionally not
        # imposed on nearly isotropic objects where that direction is unstable.
        ...
        if anisotropy >= 0.14:
            ...
            work_img = cv2.warpAffine(
                image, rot, (image.shape[1], image.shape[0]),
                flags=cv2.INTER_LINEAR,
                borderMode=cv2.BORDER_CONSTANT,
                borderValue=bg,
            )
            work_mask = cv2.warpAffine(
                mask, rot, (mask.shape[1], mask.shape[0]),
        ...
        square_i = np.empty((side, side, 3), dtype=np.uint8)
        square_i[:] = bg
    ...
    def learn(self, image: np.ndarray, attention_mask: np.ndarray) -> int:
        """Unsupervised competitive local-patch learning."""
        norm_i, norm_m = self._prepare(image, attention_mask)
        updates = 0
        for patch, _ in self._patches(norm_i, norm_m):
            if not self.codewords:
                self.codewords.append(patch.copy())
                self.codeword_counts.append(1)
                updates += 1
                continue
            rows = [(self._rms(patch, proto), i)
                    for i, proto in enumerate(self.codewords)]
            dist, idx = min(rows)
    ...
    def extract(self, image: np.ndarray, attention_mask: np.ndarray) -> np.ndarray:
        """Read-only descriptor extraction; never mutates the codebook."""
        norm_i, norm_m = self._prepare(image, attention_mask)
        focus = norm_m > 32
        pix = norm_i[focus]
        if len(pix) < 4:
        ...
        pix01 = pix.astype(np.float64) / 255.0
        photometric_parts = []
        for ch in range(3):
            hist, _ = np.histogram(
                pix[:, ch], bins=self.histogram_bins, range=(0, 256))
            hist = hist.astype(np.float64)
            hist /= max(float(hist.sum()), 1.0)
            photometric_parts.append(hist)

        # Continuous generic intensity statistics prevent coarse histogram bins
        # from erasing differences that matter to whatever concepts language
        # later discovers. No dimension is tagged with a color word.
        photometric_parts.append(pix01.mean(axis=0))
        photometric_parts.append(pix01.std(axis=0))

        sums = np.maximum(pix01.sum(axis=1, keepdims=True), 1e-8)
        chrom = pix01 / sums
        photometric_parts.append(chrom.mean(axis=0))
        photometric_parts.append(chrom.std(axis=0))
        ...
        photometric_parts.append(joint.reshape(-1))
        pixel_block = np.concatenate(photometric_parts)

        raster = cv2.resize(
            norm_m, (self.raster_size, self.raster_size),
            interpolation=cv2.INTER_AREA,
        ).astype(np.float64).reshape(-1) / 255.0
    ...
    def from_dict(cls, data: Dict[str, object]) -> "SelfOrganizingPatchSensor":
        obj = cls(
            ...
            chromatic_bins=int(data.get("chromatic_bins", 10)),
        ...
        obj.codewords = [np.asarray(x, dtype=np.float64)
                         for x in data.get("codewords", [])]
